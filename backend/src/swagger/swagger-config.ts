...
export const frontendAuthStorageKey = 'auth-storage';

export function buildSwaggerCustomJs(): string {
  return `
    (function () {
      ...
      var frontendAuthStorageKey = '${frontendAuthStorageKey}';
      var pollTimer = null;
      ...
      function getFrontendSessionToken() {
        var frontendAuthStorage = localStorage.getItem(frontendAuthStorageKey);
        if (!frontendAuthStorage) {
          return null;
        }

        try {
          var parsedStorage = JSON.parse(frontendAuthStorage);
          var persistedState = parsedStorage && parsedStorage.state ? parsedStorage.state : parsedStorage;
          return persistedState && typeof persistedState.sessionToken === 'string'
      ...
      function syncAuthUi(ui) {
        ...
        var token = getAuthorizedToken(ui) || localStorage.getItem(storageKey) || getFrontendSessionToken();
        if (token) {
          applyToken(ui, token);
        }
