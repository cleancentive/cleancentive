---
name: triagato
description: Fetch open feedback from PROD, sync local plans, classify and analyze new items. Use when you need to triage user feedback into actionable implementation plans.
user-invocable: true
allowed-tools:
  - read_file
  - write_file
  ...
  - bash
  - ask_user_question
compatibility: Requires curl, jq, and access to the cleancentive API
---
...
- This command writes to upstream (status changes and steward comments) — every write is gated on an explicit user confirmation
